...
function StarRating({ rating }) {
  return (
    <div className="flex items-center gap-1">
      {[1,2,3,4,5].map((s) => (
        <svg key={s} className={`w-4 h-4 ${s <= Math.round(rating)
...
export default function ArtisanModal({ Artisan, onClose, onSaveToggle }) {
  ...
  return (
    <div
      className="fixed inset-0 z-50 flex items-center justify-center px-3 sm:px-4 py-4"
      style={{ background: "rgba(0,0,0,0.5)", backdropFilter: "blur(4px)" }}
      onClick={(e) => e.target === e.currentTarget && onClose()}
    >
      <div
        className="bg-white dark:bg-dark-surface rounded-2xl w-full max-w-md max-h-[calc(100vh-2rem)] overflow-y-auto"
      >
        {/* Header — green banner */}
        <div className="bg-green-500 px-4 sm:px-6 py-6 sm:py-8 relative">
          <button
            onClick={onClose}
          ...
          <div className="flex items-center gap-4 min-w-0">
            ...
            <div className="min-w-0 pr-8">
              ...
              <span className="bg-white/20 text-white text-xs font-semibold 
                               px-2.5 py-0.5 rounded-full mt-1 inline-block">
                {Artisan.category}
              </span>
            </div>
          </div>
        ...
        <div className="px-4 sm:px-6 py-5">
          ...
          <div className="flex flex-col gap-3 mb-5">
            {[
              ...
              {
                label: "Member Since",
                value: new Date(Artisan.createdAt).toLocaleDateString("en-NG", {
          ...
          <div className="flex flex-col gap-2">
            <div className="flex flex-col sm:flex-row gap-2">
              ...
              <a href={`https://wa.me/${Artisan.phone?.replace(/^0/, "234")}`}
            ...
            <button
              ...
              className={`w-full flex items-center justify-center gap-2 py-3 
                         ...
                         ${saved
                           ? "bg-red-50 dark:bg-red-500/10 text-red-500 border-red-200"
                           : "bg-green-50 dark:bg-green-500/10 text-green-500 border-green-200"
                         }`}
            >
              ...
              {saving ? "Saving..." : saved ? "Remove from Saved" : "Save Artisan"}
            </button>
          </div>
        </div>
      </div>
