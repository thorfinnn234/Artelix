...
import { getSavedArtisan } from "../../services/artisanService";
import UserLayout from "../../layouts/UserLayout";
import ArtisanCard from "../../components/ArtisanCard";

export default function SavedArtisan() {
  ...
  const fetchSaved = async () => {
    ...
    setError("");
    try {
      const data = await getSavedArtisan();
      const list = Array.isArray(data)
        ? data
        : data.items || data.saved || data.Artisan || [];
      setArtisan(list);
    } catch (err) {
      setError("Failed to load saved Artisan.");
    } finally {
      setLoading(false);
    }
  };
    ...
    <UserLayout>
      <div className="mb-6">
        ...
        <p className="text-gray-500 dark:text-gray-400 text-sm mt-1">
          Artisan you've bookmarked for later
        </p>
      </div>
      ...
      {!loading && Artisan.length > 0 && (
        <>
          <p className="text-xs text-gray-400 mb-4">
            {Artisan.length} saved Artisan{Artisan.length !== 1 ? "s" : ""}
          </p>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {Artisan.map((item) => {
      ...
      {!loading && Artisan.length === 0 && (
        <div className="flex flex-col items-center justify-center py-20">
          ...
          <h3 className="font-heading font-semibold text-gray-900 
            ...
            No saved Artisan yet
          </h3>
          <p className="text-gray-400 text-sm text-center max-w-xs mb-4">
            Browse Artisan and tap the heart icon to save them here
          </p>
