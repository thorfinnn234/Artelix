...
async function ensureAdmin() {
  // Create or find an admin owner for seeded Artisan
  const email = "seed-admin@Artelix.dev";
...
async function main() {
  ...
  // Optional: clear Artisan first
  const CLEAR_FIRST = true;
  if (CLEAR_FIRST) {
    await Artisan.deleteMany({});
    console.log("🧹 Cleared existing Artisan");
  }

  const total = 50;
  const statuses = ["approved", "pending", "suspended"];

  const Artisan = Array.from({ length: total }).map((_, i) => {
    const category = rand(categories);
    const name = `Artelix ${category} ${i + 1}`;
  ...
  await Artisan.insertMany(Artisan);
  console.log(`🌱 Seeded ${total} Artisan`);
